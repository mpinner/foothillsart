...
INPUT_CSV_FILE = ".\\load_artshow.csv"

## JUROR SHOW SETTING
IS_JUROR_SHOW = True
SHOW_TITLE = "foothills October2023"
OUTPUT_JUROR_DIR=".\\LOADED_ARTSHOW_JUROR_OUTPUT"

IMAGE_DIR=".\\LOADED_ARTSHOW_IMAGES"
OUTPUT_DIR=".\\LOADED_ARTSHOW_WEB_OUTPUT"
...
create_clean_directory.removeAndCreate(OUTPUT_JUROR_DIR)
...
def loadArtwork(entry, submissionNum, index):
    ...
    if titleKey in entry.keys():
        ...
        if not pd.isna(title):
            ...
            ## calculate a juror artwork number
            number = index * 4 + submissionNum
            ...
            if not pd.isna(title):
                ...
                if not pd.isna(url):
                    ...
                    if IS_JUROR_SHOW:
                        createjurorimages.createjurorimages(
                            filename, 
                            title=title,
                            medium=medium,
                            dimension=dimensions,
                            ...
                            folder=IMAGE_DIR, 
                            output_folder=OUTPUT_JUROR_DIR)
...
for index, entry in artshow.iterrows():
    if isValid(entry):
        ...
        for submissionNum in range(1, 5):
            loadArtwork(entry, submissionNum, index)
